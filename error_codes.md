...
|------|-------------|-------------|-------------|-----------------|
| `INVALID_REQUEST` | 400 | Malformed JSON, missing fields, missing X-Player-Id, invalid enum values. | false | Show "Technical error" and stop. Log details. No retry. |
| `INVALID_BET` | 400 | betAmount not in allowedBets from /init. | false | Show "Bet not allowed". Disable spin until bet changed. |
| `FEATURE_DISABLED` | 409 | Client requested feature that is disabled by server configuration returned from /init (e.g., BUY_FEATURE or Hype Mode). | false | Hide/disable feature UI; show toast. |
...
| `NOT_IMPLEMENTED` | 501 | DEV ONLY: Endpoint exists but logic not yet implemented (scaffold only). MUST NOT be present in production/audit builds. | false | Show "Coming soon". Do not retry. |
...
- For `INTERNAL_ERROR`: auto-retry is allowed ONLY if backend idempotency (clientRequestId cache) is enabled and guarantees no double-charge.
